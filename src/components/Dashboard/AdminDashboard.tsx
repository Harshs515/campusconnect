...
import { Users, Briefcase, FileText, Award, ArrowRight, Shield, TrendingUp } from 'lucide-react';
...
const AdminDashboard: React.FC = () => {
  ...
  useEffect(() => {
    if (!user?.id) return;
    fetch(`${API}/analytics/overview`, { headers: getHeaders() })
  ...
  const getCount = (arr: any[], key: string) =>
    parseInt(arr?.find((i: any) => i.role === key || i.status === key)?.count || '0');

  const totalUsers = data?.users?.reduce((s: number, u: any) => s + parseInt(u.count), 0) || 0;
  const totalJobs = data?.jobs?.reduce((s: number, j: any) => s + parseInt(j.count), 0) || 0;
  const totalApps = data?.applications?.reduce((s: number, a: any) => s + parseInt(a.count), 0) || 0;
    ...
    <div className="font-sans space-y-8 pb-12 w-full max-w-7xl mx-auto">
      ...
      <div className="bg-[#062817] rounded-[32px] p-8 sm:p-10 relative overflow-hidden flex flex-col md:flex-row justify-between items-center shadow-lg">
        ...
        <div className="relative z-10 w-full md:w-2/3 mb-8 md:mb-0">
          <p className="text-emerald-300 font-bold uppercase tracking-wider text-sm mb-2">Admin Console</p>
           ...
           <div className="bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl p-6 shadow-2xl transform rotate-2 hover:rotate-0 transition-transform duration-500 w-64">
              ...
              <div className="space-y-2">
                 <div className="h-2 bg-white/10 rounded-full overflow-hidden">
                    <div className="h-full bg-emerald-400 w-11/12 rounded-full"></div>
                 </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
        {[
          { label: 'Total Users', value: totalUsers, icon: Users, color: 'text-blue-600', dot: 'bg-blue-500' },
          { label: 'Active Jobs', value: getCount(data?.jobs, 'active'), icon: Briefcase, color: 'text-emerald-600', dot: 'bg-emerald-500' },
          { label: 'Applications', value: totalApps, icon: FileText, color: 'text-purple-600', dot: 'bg-purple-500' },
          { label: 'Alumni Placed', value: data?.alumniCount || 0, icon: Award, color: 'text-amber-600', dot: 'bg-amber-500' },
        ].map(stat => {
